...
# Function to interact with Dialogflow API and generate responses
...
if uploaded_file is not None:
    ...
    for index, row in preprocessed_df.iterrows():
        ...
        response = detect_intent_text(project_id, agent=agent, location=location, session_id=session_id, text=question, event_name=event_name, parameters=parameters)
        
        # Append the question and corresponding answer to the result DataFrame
        result_df = result_df._append({'question': question, 'response': response}, ignore_index=True)
    ...
    csv_file_path = "dialogflow_results.csv"  # Specify the correct path to the CSV file
